...
class LandingConfig(Config):
    ...
    NUM_CLASSES = 1 + 2  # Background + class names
...
class LandingDataset(utils.Dataset):
    ...
    def load_mask(self, image_id):
        ...
        return mask.astype(np.bool), np.array(info['class_ids'], dtype=np.int32) #np.ones([mask.shape[-1]], dtype=np.int32)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Train Mask R-CNN to detect safe landing areas.')
    ...
    elif args.weights.lower() == "landing":
        # Find last trained weights
        weights_path = LANDING_MODEL_PATH
    else:
